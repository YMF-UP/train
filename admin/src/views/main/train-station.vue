<template>
  <p>
    <a-space>
      ...
      <a-button type="primary" @click="handleQuery({page: 1, size: pagination.pageSize})">查找</a-button>
  ...
  <a-modal v-model:visible="visible" title="火车车站" @ok="handleOk"
    ...
    <a-form :model="trainStation" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      <a-form-item label="车次编号">
        <train-select-view v-model="trainStation.trainCode" width="100%"></train-select-view>
      </a-form-item>
      <a-form-item label="站序">
        <a-input v-model:value="trainStation.index" />
      </a-form-item>
      <a-form-item label="站名">
        <station-select-view v-model="trainStation.name" width="100%"></station-select-view>
      </a-form-item>
      ...
      <a-form-item label="停站时长">
        <a-time-picker v-model:value="trainStation.stopTime" valueFormat="HH:mm:ss" placeholder="自动计算" disabled />
      </a-form-item>
...
export default defineComponent({
  ...
  setup() {
    ...
    // 监听进站与出站时间变化，通过 dayjs 自动计算停站时长
    watch(() => [trainStation.value.inTime, trainStation.value.outTime], () => {
      if (window.Tool.isNotEmpty(trainStation.value.inTime) && window.Tool.isNotEmpty(trainStation.value.outTime)) {
        let diff = dayjs('2000-01-01 ' + trainStation.value.outTime).diff(dayjs('2000-01-01 ' + trainStation.value.inTime), 'seconds');
        if (diff >= 0) {
          trainStation.value.stopTime = dayjs('2000-01-01 00:00:00').second(diff).format('HH:mm:ss');
        }
      }
    });
