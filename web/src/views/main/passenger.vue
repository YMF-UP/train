  ...
  <a-table :dataSource="passengers"
    ...
    <template #bodyCell="{ column, record }">
      ...
      <template v-else-if="column.dataIndex === 'type'">
        <span v-for="item in PASSANGER_TYPE_ARRAY" :key="item.code">
    ...
    <a-form :model="passenger" :label-col="{span: 4}" :wrapper-col="{ span: 20 }">
      ...
      <a-form-item label="旅客类型">
        <a-select v-model:value="passenger.type">
          <a-select-option v-for="item in PASSANGER_TYPE_ARRAY" :key="item.code" :value="item.code">
            {{item.desc}}
          </a-select-option>
        </a-select>
      </a-form-item>
...
export default defineComponent({
  name: "passenger-view",
  setup() {
    const PASSANGER_TYPE_ARRAY = window.PASSANGER_TYPE_ARRAY;
